...
ConnectionPool.prototype.acquire = function (callback) {
    ...
    console.log("Acquiring connection: ", this.connections.map(function(connection) { return [connection.id, connection.status]; }));
    if (free === undefined) { //no valid connection found
        console.log('There are ' + this.connections.length + ' connections');
...
function setFree(pooled) {
    ...
    console.log('Setting connection as free: ' + pooled.id);
    pooled.status = FREE;
...
ConnectionPool.prototype.release = function(connection) {
    ...
    console.log("Looking for a connection to release: ", this.connections.map(function(connection) { return [connection.id, connection.status]; }));
    for (i = this.connections.length - 1; i >= 0; i--) {
        pooled = this.connections[i];

        if (pooled.con === connection) {
        	console.log('Index: ' + i);
            console.log('Found the connection to be reset: ' + pooled.id);
            ...
            connection.reset(function (err) {
                ...
                if (!pooled.con) {//the connection failed during the reset
                    console.log("No pooled con");
                    return;
                }

                if (err) { //there is an error, don't reuse the connection, just close it
                    console.log('Closing the connection and getting out');
                    pooled.con.close();
                    return;
                }
                console.log('Connection reset: ' + pooled.id);
                self.log('connection reset: ' + pooled.id);

                var waiter = self.waiting.shift();

                if (waiter !== undefined) {
                	console.log("Will set to used", waiter);
                    setUsed.call(self, pooled, waiter);
                    //if (waiter.timeout)
                    //    clearTimeout(waiter.timeout);
                    //waiter.callback(null, connection);
                } else {
                	console.log("Will set to free");
                    setFree.call(self, pooled);
                }
            });
